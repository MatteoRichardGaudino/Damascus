...
void engine_config_init_default(EngineConfig *cfg) {
    ...
    cfg->mcts_exploration = 0.6422f;
    cfg->mcts_max_rollout_depth = 51;
    cfg->mcts_rollout_epsilon = 0.3128f;
    cfg->mcts_use_db = false;
    cfg->mcts_use_book = true;
    cfg->mcts_debug_log = false;
    cfg->puct_time_budget = 1.0;
    cfg->puct_c_puct = 2.5857f;
    cfg->puct_temperature = 2.2048f;
    cfg->puct_max_rollout_depth = 112;
    cfg->puct_rollout_epsilon = 0.3416f;
    cfg->puct_use_db = false;
    cfg->puct_use_guided_book = true;
    cfg->puct_lambda_book = 0.0791f;
    cfg->puct_use_book = false;
    cfg->puct_debug_log = false;
    cfg->book_backend = BOOK_BACKEND_KINGSROW_ODB;
    cfg->book_mode = BOOK_MODE_GOOD;
    cfg->book_temperature = 0.0500f;
...
void engine_apply_config(Engine *engine, EngineType type, const EngineConfig *cfg) {
    ...
    switch (type) {
        ...
        case ENGINE_TYPE_MCTS_PUCT:
            ...
            engine_mcts_puct_set_guided_book(engine->internal_state, cfg->puct_use_guided_book, cfg->puct_lambda_book);
            engine_mcts_puct_set_use_book(engine->internal_state, (!cfg->puct_use_guided_book) && cfg->puct_use_book && (cfg->book_mode != BOOK_MODE_OFF));
            engine_mcts_puct_set_book_mode(engine->internal_state, cfg->puct_use_guided_book ? BOOK_MODE_OFF : cfg->book_mode);
